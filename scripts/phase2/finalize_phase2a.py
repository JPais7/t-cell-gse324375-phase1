...
bio=pd.DataFrame([
 ...
 {'candidate':C2,'canonical_relationship':'CD28-CD86 costimulation exists but ranking orientation is associative','direct_mechanistic_support':'Known costimulatory biology; literal ligand/receptor orientation not assumed','interaction_class':'ACTIVATION_STATE_ASSOCIATION','orientation_status':'BIOLOGICAL_REINTERPRETATION_REQUIRED','biological_interpretation':'T-cell activation-state coupling with CD8 state','main_concern':'Negative association and non-causal ranking semantics','status':'BIOLOGICALLY_PLAUSIBLE_WITH_REINTERPRETATION'},
 {'candidate':C3,'canonical_relationship':'NOT_ASSESSABLE','direct_mechanistic_support':'NOT_ASSESSABLE','interaction_class':'NOT_ASSESSABLE','orientation_status':'NOT_ASSESSABLE','biological_interpretation':'','main_concern':'0 complete candidate-axis cases','status':'NOT_ASSESSABLE'}]); bio.to_csv(O/'top3_biological_plausibility.tsv',sep='\t',index=False)
rows=[]
for c in [C1,C2,C3]:
