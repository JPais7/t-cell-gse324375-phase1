...
if src.exists() and bio.exists():
 ...
 if eff.exists() and lo.exists():
  e=pd.read_csv(eff,sep='\t'); s=e.groupby('candidate').effect_value.agg(n_positive=lambda x:int((x>0).sum()),n_negative=lambda x:int((x<0).sum()),n_zero_or_undetermined=lambda x:int((x==0).sum()),n_mice='count').reset_index(); s['majority_direction']=s.apply(lambda r:'positive' if r.n_positive>r.n_negative else ('negative' if r.n_negative>r.n_positive else 'balanced'),axis=1); s['majority_direction_fraction']=s[['n_positive','n_negative']].max(axis=1)/s.n_mice; s['two_sided_direction_strength']=2*(s.majority_direction_fraction-.5).abs(); l=pd.read_csv(lo,sep='\t').drop(columns=[c for c in ['n_positive','n_negative','n_zero_or_undetermined','majority_direction','majority_direction_fraction','two_sided_direction_strength'] if c in pd.read_csv(lo,sep='\t',nrows=0).columns]); l.merge(s,on='candidate',how='left').to_csv(lo,sep='\t',index=False)
