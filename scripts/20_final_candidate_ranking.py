...
ready=allc.copy(); ready["statistical_status"]=np.where((ready.direction_consistency.fillna(0)>=.5)&(ready.effect_component>=.1),"PASS","FAIL"); ready["biological_status"]=np.where(ready.biological_validity_component>=.8,"PASS","FAIL"); ready["mechanistic_status"]=np.where(ready.direction_mechanistically_concordant& (ready.mechanistic_evidence_coherence>=.2),"PASS","HOLD"); ready["experimental_status"]=np.where(ready.experimental_testability_component==1,"PASS","FAIL"); ready["LOOCV_status"]=np.where(ready.LOOCV_stability.fillna(0)>=.5,"PASS","FAIL"); ready["null_status"]=np.where(ready.empirical_p.fillna(1)<=.2,"PASS","FAIL"); ready["external_evidence_status"]=ready.external_perturbation_support; ready["phase2_decision"]=np.where(ready.evidence_gate,"GO",np.where((ready.biological_status.eq("PASS"))&(ready.statistical_status.eq("PASS")),"HOLD","REJECT")); ready["decision_reason"]=np.where(ready.phase2_decision.eq("GO"),"passes biological, direction, animal-level, LOOCV, null and testability gates",np.where(ready.phase2_decision.eq("REJECT"),"critical biological or statistical gate failed",np.where(~ready.direction_mechanistically_concordant,"HOLD: molecular axis valid but RNA/ADT direction is discordant","plausible but one or more robustness/mechanistic gates remain incomplete"))); readiness=ready[["candidate","candidate_type","statistical_status","biological_status","mechanistic_status","experimental_status","LOOCV_status","null_status","external_evidence_status","phase2_decision","decision_reason"]];
if readiness.empty: raise RuntimeError("phase2_readiness.tsv would be empty")
if not {"candidate","candidate_type","phase2_decision"}.issubset(readiness.columns): raise RuntimeError("phase2_readiness missing required columns")
readiness.to_csv(OUT/"phase2_readiness.tsv",sep="\t",index=False)
if pd.read_csv(OUT/"phase2_readiness.tsv",sep="\t").empty: raise RuntimeError("phase2_readiness.tsv was written but is empty")
with (OUT/"TOP3_candidates.md").open("w") as h:
 ...
 if top3.empty:
  lead=allc.iloc[0]; h.write(f"No candidate passes every closed Phase 2 readiness gate.\n\n## Leading HOLD: {lead.candidate}\n\nThis {lead.candidate_type} candidate has association/robustness evidence but is not a causal conclusion. The unresolved direction and absence of independent perturbation evidence require candidate-specific validation before Phase 2.\n")
...
r=top.iloc[0] if len(top) else allc.iloc[0]; go_summary=(f"Only {len(top)} candidate(s) pass the closed Phase 2 gate. The GO hypothesis is **{r.candidate}**." if len(top) else f"No candidate passes the closed Phase 2 gate. The leading {r.candidate_type} hypothesis, **{r.candidate}**, is HOLD because one or more biological, direction, robustness or causal-validation gates remain unresolved."); final_q=("Candidate-specific gates remain unresolved; independent perturbational validation is required." if not len(top) else "Q1–Q6: PASS. Q7 independent perturbation: NOT_ASSESSED. Q8–Q9: PASS. Q10 residual confounding remains possible. Proceed only with the GO candidate.")
