...
def score(row,values):
 ...
 expected=getattr(row,"expected_direction","UNRESOLVED")
 if expected=="POSITIVE_ACTIVATION": repl=min(len(v)/10,1)*float((v>0).mean())
 elif expected=="NEGATIVE_ACTIVATION": repl=min(len(v)/10,1)*float((v<0).mean())
 else: repl=min(len(v)/10,1)*float(2*max(abs(max((v>0).mean(),(v<0).mean())-.5),0))
...
for c,row in base.iterrows():
 ...
 v=np.asarray(list(vals.values()),float); observed=score(row,vals); expected=getattr(row,"expected_direction","UNRESOLVED")
 if len(v)>=2:
  ns=[]
  for _ in range(NPERM): ns.append(score(row,{str(i):x*s for i,(x,s) in enumerate(zip(v,rng.choice([-1,1],len(v))))}))
  ns=np.asarray(ns); uniq=np.unique(np.round(ns,12)); p=(1+(ns>=observed).sum())/(NPERM+1); ntype="DIRECTIONAL_SIGN_NULL" if expected!="UNRESOLVED" else "TWO_SIDED_SIGN_NULL"; status="DEGENERATE" if len(uniq)<=1 else ("PASS" if p<.05 else "FAIL")
  nullrows.append({"candidate":c,"candidate_family_id":row.candidate_family_id,"null_test_type":ntype,"expected_direction":expected,"observed_statistic":observed,"observed_score":observed,"null_mean":ns.mean(),"null_sd":ns.std(),"null_95":np.quantile(ns,.95),"null_99":np.quantile(ns,.99),"empirical_p":p,"null_percentile":(ns<=observed).mean(),"null_p95":np.quantile(ns,.95),"null_p99":np.quantile(ns,.99),"n_unique_null_scores":len(uniq),"fraction_identical_to_observed":float(np.mean(np.isclose(ns,observed))),"n_permutations":NPERM,"random_seed":SEED,"null_status":status,"null_model_type":"within-candidate mouse-effect sign permutation"})
